...
export default async function MemberDashboard() {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', padding: '0 0 20px 0' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px', padding: '0 20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
                    ...
                    <div className="card" style={{ 
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.1)', padding: '10px 16px', borderRadius: '16px', margin: '0 -4px' }}>
                            ...
                            <a 
                                href={`https://wa.me/${user.gym?.whatsappNumber?.replace(/\D/g, '') || ''}?text=${encodeURIComponent(`Hi ${user.gym?.name || 'PulseFit'}! I want to renew my membership.\n\nName: ${user.name}\nID: #${user.id.slice(-8).toUpperCase()}\nPlan: ${activeMembership?.plan.name || 'N/A'}`)}`}
                                target="_blank"
                                rel="noopener noreferrer"
                                style={{ background: '#fff', color: '#000', padding: '8px 14px', borderRadius: '12px', textDecoration: 'none', fontSize: '0.7rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '6px' }}
                            >
